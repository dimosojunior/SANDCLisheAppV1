  ...
  return (
    ...
    initialRouteName="Home Tab"
     ...
     options={ ({route}) => ({ 
      ...
      options={ ({route}) => ({
      //Hii inatumika kutoa tab bar style in signin stack 
      	 tabBarStyle:  {
		...
		backgroundColor:'yellow'
         ...
         tabBarIcon: ({focused}) => (
           ...
           //header: () => <MainHeader  />,
           header:() => removeHeaderVisibility(route),
...
const getTabBarVisibility = route => {
	...
	if (routeName === 'Onboarding Stack' || routeName === 'Signin Stack' || routeName === 'Signup Stack') {
		return 'none';
	}
	return 'flex';
...
const removeHeaderVisibility = route => {
	console.log(route);
	const screenName = getFocusedRouteNameFromRoute(route) ?? 'Feed';
	const screenName2 = getFocusedRouteNameFromRoute(route) ?? 'Feed';
	console.log(screenName);

	if (screenName === 'Onboarding Stack' || screenName === 'Signin Stack' || screenName === 'Signup Stack') {
		return null;
	}
	return <MainHeader />;
