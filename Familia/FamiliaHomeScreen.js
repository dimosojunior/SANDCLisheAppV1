...
import FamiliaWeeksRenderedScreen from '../Screens/FamiliaWeeksRenderedScreen';


const FamiliaHomeScreen = ({navigation}) => {
...
//     ]);
 ...
 //FOR  APIS
const { datas:JeWajua } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/JeWajuaFamiliaWeek1/');
...
//FOR  APIS
const { datas:MatatizoUnayowezaKupata } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/MatatizoUnayowezaKupataFamiliaWeek1/');
...
//FOR  APIS
const { datas:TaarifaZaFamilia } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/TaarifaZaFamiliaWeek1/');
...
// const [Familia, setWajawazito] = useState([
//   {
//     Time:'BREAKFAST',
//     Description:'Description 1',
...
//     ]);
 ...
 //FOR  APIS
const { datas:Familia, isPending, error } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/FamiliaWeek1/');
  ...
  const TaarifaZaFamiliaCard = ({data}) => {
    return (
      <TouchableOpacity
...
{Familia && Familia.length > 0 ? (
                ...
                style={globalStyles.MainContainerListFilterContainer}
                 >
                  ...
                  {Familia.map(data => {
                    return <FamiliaCard data={data} key={data.id} />;
                  })}
                </View>
...
{/*TAARIFA ZA UJAUZITO*/}
 ...
 {TaarifaZaFamilia && TaarifaZaFamilia.length > 0 ? (
          <View
        style={globalStyles.MainContainerNdogo}
          >
          <View
          ...
          style={globalStyles.MainContainerListFilterContainer}
           >
            ...
            {TaarifaZaFamilia.map(data => {
              return <TaarifaZaFamiliaCard data={data} key={data.id} />;
            })}
          </View>
...
export default FamiliaHomeScreen;
