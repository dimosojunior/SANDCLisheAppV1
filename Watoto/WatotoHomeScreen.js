...
import WatotoWeeksRenderedScreen from '../Screens/WatotoWeeksRenderedScreen';


const WatotoHomeScreen = ({navigation}) => {
...
//     ]);
 ...
 //FOR  APIS
const { datas:JeWajua } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/JeWajuaWatotoWeek1/');
...
//FOR  APIS
const { datas:MatatizoUnayowezaKupata } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/MatatizoUnayowezaKupataWatotoWeek1/');
...
//FOR  APIS
const { datas:TaarifaZaWatoto } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/TaarifaZaWatotoWeek1/');
...
// const [Watoto, setWajawazito] = useState([
//   {
//     Time:'BREAKFAST',
//     Description:'Description 1',
...
//     ]);
 ...
 //FOR  APIS
const { datas:Watoto, isPending, error } = useFetch('https://lisheapisapp.pythonanywhere.com/Lishe/WatotoWeek1/');
  ...
  const TaarifaZaWatotoCard = ({data}) => {
    return (
      <TouchableOpacity
...
{Watoto && Watoto.length > 0 ? (
                ...
                style={globalStyles.MainContainerListFilterContainer}
                 >
                  ...
                  {Watoto.map(data => {
                    return <WatotoCard data={data} key={data.id} />;
                  })}
                </View>
...
{/*TAARIFA ZA UJAUZITO*/}
 ...
 {TaarifaZaWatoto && TaarifaZaWatoto.length > 0 ? (
          <View
        style={globalStyles.MainContainerNdogo}
          >
          <View
          ...
          style={globalStyles.MainContainerListFilterContainer}
           >
            ...
            {TaarifaZaWatoto.map(data => {
              return <TaarifaZaWatotoCard data={data} key={data.id} />;
            })}
          </View>
...
export default WatotoHomeScreen;
